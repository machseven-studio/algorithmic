...
DEFAULT_PERMS = {m: "none" for m in VALID_MODULES}
DEFAULT_PERMS.update({"students":"edit","teachers":"edit","classrooms":"edit","syllabus":"edit","timetable":"edit","attendance":"edit","invigilation":"edit","fees":"edit","seating":"edit"})

if not DATABASE_URL:
    raise RuntimeError("DATABASE_URL is required. Configure a PostgreSQL database in Render/local environment.")
...
def q(sql, params=(), one=False, all_rows=False):
    with db() as c:
        with c.cursor() as cur:
            ...
            if all_rows: return cur.fetchall()
            return None
...
def init_db():
    with db() as c:
        ...
        c.execute("CREATE TABLE IF NOT EXISTS staff_users (id BIGSERIAL PRIMARY KEY, institute_id BIGINT NOT NULL REFERENCES institutes(id) ON DELETE CASCADE, full_name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, password_salt TEXT NOT NULL, designation TEXT NOT NULL, permissions JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ NOT NULL DEFAULT now())")
        ...
        # Backward-compatible additions for older PostgreSQL deployments.
...
def hash_pw(password, salt): return hashlib.pbkdf2_hmac("sha256", password.encode(), bytes.fromhex(salt), 200_000).hex()
...
def current(auth: str = Header(None)):
    if not auth or not auth.startswith("Bearer "): raise HTTPException(401,"Not authenticated")
    ...
    if not s or s["expires_at"] < datetime.now(IST): raise HTTPException(401,"Invalid or expired session")
    if s["staff_user_id"]:
        u=q("SELECT * FROM staff_users WHERE id=%s",(s["staff_user_id"],),one=True)
        if not u: raise HTTPException(401,"Invalid staff account")
...
def owner(c=Depends(current)):
    if not c["is_owner"]: raise HTTPException(403,"Only the boss can manage users")
    return c

def access(module, write=False):
    def dep(c=Depends(current)):
        level="edit" if c["is_owner"] else (c["permissions"] or {}).get(module,"none")
        if level not in (["edit"] if write else ["view","edit"]): raise HTTPException(403,f"No access to {module}")
        return c
    return dep

def branch_ok(branch_id, institute_id):
    if not q("SELECT id FROM branches WHERE id=%s AND institute_id=%s",(branch_id,institute_id),one=True): raise HTTPException(404,"Branch not found")

class Signup(BaseModel): institute_name:str; full_name:str; email:EmailStr; password:str
class Login(BaseModel): email:EmailStr; password:str
class StaffCreate(BaseModel): full_name:str; email:EmailStr; password:str; designation:str; permissions:dict[str,str]|None=None
class StaffUpdate(BaseModel): full_name:str|None=None; email:EmailStr|None=None; password:str|None=None; designation:str|None=None; permissions:dict[str,str]|None=None
class BranchIn(BaseModel): name:str
class Record(BaseModel): data:dict
class AttendanceIn(BaseModel): student_id:int|None=None; student_name:str; batch:str|None=None; date:date; status:str
class TimetableIn(BaseModel): batch_name:str; days:list[str]; unavailable_days:list[str]=[]; slots:list[str]; lectures_per_week:int; subjects:list[dict]; teachers:list[str]=[]; rooms:list[str]=[]
class TimetableEdit(BaseModel): day:str; time_slot:str; lecture_number:int; subject:str|None=None; teacher:str|None=None; room:str|None=None
...
def signup(x:Signup):
    ...
    try:
        with db() as c:
            r=c.execute("INSERT INTO institutes(institute_name,full_name,email,password_hash,password_salt) VALUES(%s,%s,%s,%s,%s) RETURNING id",(x.institute_name,x.full_name,x.email.lower(),hash_pw(x.password,salt),salt)).fetchone(); iid=r["id"]
            c.execute("INSERT INTO branches(institute_id,name) VALUES(%s,'Main Campus')",(iid,)); c.commit()
    except Exception: raise HTTPException(400,"An account with this email already exists")
...
def login(x:Login):
    ...
    if u and secrets.compare_digest(hash_pw(x.password,u["password_salt"]),u["password_hash"]): return {"token":session_token(u["id"]),"institute_name":u["institute_name"],"full_name":u["full_name"] or "","is_owner":True,"designation":"boss"}
...
@app.get("/api/auth/me")
def me(c=Depends(current)): return c
@app.post("/api/auth/logout")
def logout(auth:str=Header(None)):
    if auth and auth.startswith("Bearer "): q("DELETE FROM sessions WHERE token=%s",(auth.split(" ",1)[1],))
    return {"status":"logged out"}
@app.patch("/api/institute/name")
def institute_name(x:dict,c=Depends(access("students",True))):
    name=(x.get("institute_name") or "").strip()
    if not name: raise HTTPException(400,"Institute name cannot be empty")
    q("UPDATE institutes SET institute_name=%s WHERE id=%s",(name,c["id"])); return {"institute_name":name}

@app.get("/api/branches")
def branches(c=Depends(current)): return q("SELECT * FROM branches WHERE institute_id=%s ORDER BY name",(c["id"],),all_rows=True)
@app.post("/api/branches")
def add_branch(x:BranchIn,c=Depends(access("students",True))):
    r=q("INSERT INTO branches(institute_id,name) VALUES(%s,%s) RETURNING *",(c["id"],x.name.strip()),one=True); return r

@app.get("/api/users")
def users(c=Depends(owner)): return q("SELECT id,full_name,email,designation,permissions,created_at FROM staff_users WHERE institute_id=%s ORDER BY full_name",(c["id"],),all_rows=True)
@app.post("/api/users")
def add_user(x:StaffCreate,c=Depends(owner)):
    if len(x.password)<8: raise HTTPException(400,"Password must be at least 8 characters")
    perms=x.permissions or {m:"none" for m in VALID_MODULES}; salt=secrets.token_hex(16)
    try: return q("INSERT INTO staff_users(institute_id,full_name,email,password_hash,password_salt,designation,permissions) VALUES(%s,%s,%s,%s,%s,%s,%s) RETURNING id,full_name,email,designation,permissions,created_at",(c["id"],x.full_name,x.email.lower(),hash_pw(x.password,salt),salt,x.designation,Jsonb(perms)),one=True)
    except Exception: raise HTTPException(400,"A user with this email already exists")
...
def edit_user(uid:int,x:StaffUpdate,c=Depends(owner)):
    ...
    for k in ["full_name","email","designation"]:
        v=getattr(x,k)
        if v is not None: fields.append(k+"=%s"); vals.append(str(v).lower() if k=="email" else v)
    if x.permissions is not None: fields.append("permissions=%s"); vals.append(Jsonb(x.permissions))
    if x.password: salt=secrets.token_hex(16); fields += ["password_hash=%s","password_salt=%s"]; vals += [hash_pw(x.password,salt),salt]
    if fields: vals += [uid,c["id"]]; q("UPDATE staff_users SET "+",".join(fields)+" WHERE id=%s AND institute_id=%s",vals)
    return q("SELECT id,full_name,email,designation,permissions,created_at FROM staff_users WHERE id=%s",(uid,),one=True)
@app.delete("/api/users/{uid}")
def del_user(uid:int,c=Depends(owner)): q("DELETE FROM staff_users WHERE id=%s AND institute_id=%s",(uid,c["id"])); return {"ok":True}
...
TABLES={
 ...
 "fees":("fees",["student_name","batch","amount_inr","status","due_date","document"]),
...
def list_module(module:str,branch_id:int,c=Depends(current)):
    ...
    dep=access(module)(c) if callable(access(module)) else c
    branch_ok(branch_id,c["id"]); table,cols=TABLES[module]
...
def add_module(module:str,branch_id:int,x:Record,c=Depends(current)):
    if module not in TABLES: raise HTTPException(404,"Unknown module")
    if not c["is_owner"] and (c["permissions"] or {}).get(module)!="edit": raise HTTPException(403,"Edit access required")
    branch_ok(branch_id,c["id"]); table,cols=TABLES[module]; data={k:v for k,v in x.data.items() if k in cols}
...
def edit_module(module:str,rid:int,x:Record,branch_id:int,c=Depends(current)):
    ...
    if not data: raise HTTPException(400,"No editable fields supplied")
    keys=list(data); vals=[data[k] for k in keys]
    vals += [rid,branch_id]; q(f"UPDATE {table} SET "+",".join(k+"=%s" for k in keys)+" WHERE id=%s AND branch_id=%s",vals)
...
def delete_module(module:str,rid:int,branch_id:int,c=Depends(current)):
    if module not in TABLES: raise HTTPException(404,"Unknown module")
    if not c["is_owner"] and (c["permissions"] or {}).get(module)!="edit": raise HTTPException(403,"Edit access required")
    branch_ok(branch_id,c["id"]); table,_=TABLES[module]; q(f"DELETE FROM {table} WHERE id=%s AND branch_id=%s",(rid,branch_id)); return {"ok":True}

@app.get("/api/attendance")
def attendance(branch_id:int,c=Depends(access("attendance"))):
    branch_ok(branch_id,c["id"]); return q("SELECT id,student_id,student_name,batch,date,status,document FROM attendance WHERE branch_id=%s ORDER BY date DESC,id DESC",(branch_id,),all_rows=True)
@app.post("/api/attendance")
def add_att(x:AttendanceIn,branch_id:int,c=Depends(access("attendance",True))):
    branch_ok(branch_id,c["id"]); return q("INSERT INTO attendance(branch_id,student_id,student_name,batch,date,status) VALUES(%s,%s,%s,%s,%s,%s) RETURNING *",(branch_id,x.student_id,x.student_name,x.batch,x.date,x.status),one=True)
@app.patch("/api/attendance/{rid}")
def edit_att(rid:int,x:AttendanceIn,branch_id:int,c=Depends(access("attendance",True))):
    branch_ok(branch_id,c["id"]); q("UPDATE attendance SET student_id=%s,student_name=%s,batch=%s,date=%s,status=%s WHERE id=%s AND branch_id=%s",(x.student_id,x.student_name,x.batch,x.date,x.status,rid,branch_id)); return {"ok":True}
@app.get("/api/students/{student_id}/attendance")
def student_history(student_id:int,branch_id:int,c=Depends(access("attendance"))):
    branch_ok(branch_id,c["id"]); return q("SELECT id,date,status,batch FROM attendance WHERE student_id=%s AND branch_id=%s ORDER BY date DESC",(student_id,branch_id),all_rows=True)

DAYS=["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
def generate_timetable(cfg):
    unavailable=set(cfg.get("unavailable_days",[])); days=[d for d in cfg["days"] if d not in unavailable]; slots=cfg["slots"]
    if not days or not slots: raise HTTPException(400,"At least one available day and time slot are required")
    requested=sum(int(s.get("lectures_per_week",0)) for s in cfg["subjects"])
    if requested>len(days)*len(slots): raise HTTPException(400,"Lectures per week exceed available timetable capacity")
    teacher_set=cfg.get("teachers",[]); room_set=cfg.get("rooms",[]); out=[]; idx=0; gen=secrets.token_hex(8)
    # Greedy with per-subject weekly counts, avoiding same subject twice on a day where possible.
    counts={s["name"]:int(s.get("lectures_per_week",0)) for s in cfg["subjects"]}; names=list(counts)
    for d in days:
        used=set()
        for t in slots:
            choices=[n for n in names if counts[n]>0 and n not in used]
            if not choices: choices=[n for n in names if counts[n]>0]
            if not choices: break
            subject=sorted(choices,key=lambda n:counts[n],reverse=True)[0]; counts[subject]-=1; used.add(subject); idx+=1
    ...
    if any(v for v in counts.values()): raise HTTPException(400,"Could not satisfy all lectures-per-week prerequisites")
    return gen,out
@app.get("/api/timetable")
def timetable(branch_id:int,batch_name:str,c=Depends(access("timetable"))):
    branch_ok(branch_id,c["id"]); return q("SELECT * FROM timetable_slots WHERE branch_id=%s AND batch_name=%s ORDER BY array_position(%s::text[],day),lecture_number",(branch_id,batch_name,DAYS),all_rows=True)
@app.get("/api/timetable/config")
def timetable_config(branch_id:int,batch_name:str,c=Depends(access("timetable"))):
    branch_ok(branch_id,c["id"]); r=q("SELECT config FROM timetable_configs WHERE branch_id=%s AND batch_name=%s",(branch_id,batch_name),one=True); return r["config"] if r else None
@app.post("/api/timetable/generate")
def gen_timetable(x:TimetableIn,branch_id:int,c=Depends(access("timetable",True))):
    branch_ok(branch_id,c["id"]); cfg=x.model_dump(); gen,slots=generate_timetable(cfg)
    with db() as conn:
        ...
        for s in slots: conn.execute("INSERT INTO timetable_slots(branch_id,batch_name,day,time_slot,lecture_number,subject,teacher,room,generation_id) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)",(branch_id,x.batch_name,*s))
        conn.commit()
    return {"generation_id":gen,"slots":slots,"replaced_previous":True}
@app.patch("/api/timetable/{rid}")
def edit_timetable(rid:int,x:TimetableEdit,branch_id:int,c=Depends(access("timetable",True))):
    branch_ok(branch_id,c["id"])
    q("UPDATE timetable_slots SET day=%s,time_slot=%s,lecture_number=%s,subject=%s,teacher=%s,room=%s WHERE id=%s AND branch_id=%s",(x.day,x.time_slot,x.lecture_number,x.subject,x.teacher,x.room,rid,branch_id)); return {"ok":True}

@app.get("/api/dashboard")
def dashboard(branch_id:int,c=Depends(current)):
    branch_ok(branch_id,c["id"]); today=date.today(); monday=today-timedelta(days=today.weekday()); sunday=monday+timedelta(days=6)
    ...
    ongoing=q("SELECT batch_name,lecture_number,time_slot,subject,teacher,room FROM timetable_slots WHERE branch_id=%s AND day=%s",(branch_id,day),all_rows=True)
    current=[]
    for r in ongoing:
        try:
            a,b=[z.strip() for z in r["time_slot"].split("-",1)];
            if a<=hm<=b: current.append(r)
    ...
    return {"attendance_week":att,"fees_pending":fees,"ongoing_lectures":current,"students_by_batch":batches,"today":today.isoformat(),"now":now.isoformat()}

@app.get("/api/export/{module}")
def export_module(module:str,branch_id:int,format:str="xlsx",c=Depends(current)):
    if module not in TABLES: raise HTTPException(404,"Unknown module")
    level="edit" if c["is_owner"] else (c["permissions"] or {}).get(module,"none")
    if level=="none": raise HTTPException(403,"No access")
    branch_ok(branch_id,c["id"]); table,cols=TABLES[module]; rows=q(f"SELECT {','.join(cols)} FROM {table} WHERE branch_id=%s ORDER BY id",(branch_id,),all_rows=True)
    ...
    if format.lower()=="xlsx":
        ...
        bio=BytesIO(); wb.save(bio); bio.seek(0); return StreamingResponse(bio,media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",headers={"Content-Disposition":f'attachment; filename="{module}.xlsx"'})
    if format.lower()=="pdf":
        ...
        data=[headers]+[[str(v or "") for v in row] for row in values]; t=Table(data,repeatRows=1); t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#222222")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("GRID",(0,0),(-1,-1),0.25,colors.grey),("FONTSIZE",(0,0),(-1,-1),7),("VALIGN",(0,0),(-1,-1),"MIDDLE")])) ; story.append(t); doc.build(story); bio.seek(0); return StreamingResponse(bio,media_type="application/pdf",headers={"Content-Disposition":f'attachment; filename="{module}.pdf"'})
