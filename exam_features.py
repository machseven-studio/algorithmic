...
def write_access(institute, branch_id):
    ...
    if institute.permission == "read_only":
        raise HTTPException(status_code=403, detail="Read-only accounts cannot modify examination records")
...
def ensure_tables():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS exam_results (
            id SERIAL PRIMARY KEY,
            history_id INTEGER NOT NULL REFERENCES exam_history(id) ON DELETE CASCADE,
            branch_id INTEGER NOT NULL REFERENCES branches(id) ON DELETE CASCADE,
...
def list_results(branch_id: int, institute=Depends(main.get_current_institute)):
    access(institute, branch_id)
    conn = db(); cur = conn.cursor()
    cur.execute("SELECT id, subject, topic, batch_name, exam_date, overall_marks FROM exam_history WHERE branch_id=%s ORDER BY exam_date DESC, id DESC", (branch_id,))
    exams = [dict(r) for r in cur.fetchall()]
    for exam in exams:
        cur.execute("SELECT id, student_id, student_name, roll_number, marks FROM exam_results WHERE history_id=%s ORDER BY LOWER(student_name)", (exam["id"],))
        exam["students"] = [dict(r) for r in cur.fetchall()]
    conn.close()
    return exams
...
def create_result_exam(branch_id: int, payload: dict, institute=Depends(main.get_current_institute)):
    write_access(institute, branch_id)
    subject = str(payload.get("subject", "")).strip()
    topic = str(payload.get("topic", "")).strip()
    batch = str(payload.get("batch_name", "")).strip()
    ...
    if not all([subject, topic, batch, exam_date]) or overall <= 0:
        raise HTTPException(status_code=400, detail="Batch, subject, topic, date and positive overall marks are required")
    conn = db(); cur = conn.cursor()
    cur.execute("SELECT id FROM students WHERE branch_id=%s AND COALESCE(batch,'')=%s ORDER BY LOWER(name)", (branch_id, batch))
    students = cur.fetchall()
    if not students: conn.close(); raise HTTPException(status_code=400, detail=f"No students found in batch '{batch}'")
    cur.execute("INSERT INTO exam_history(branch_id,subject,topic,batch_name,exam_date,overall_marks) VALUES(%s,%s,%s,%s,%s,%s) RETURNING id", (branch_id,subject,topic,batch,exam_date,overall))
    history_id = cur.fetchone()[0]
    for s in students:
        cur.execute("SELECT name, roll_number FROM students WHERE id=%s", (s["id"],)); student = cur.fetchone()
        cur.execute("INSERT INTO exam_results(history_id,branch_id,student_id,student_name,roll_number) VALUES(%s,%s,%s,%s,%s)", (history_id,branch_id,s["id"],student["name"] or "",student["roll_number"] or ""))
...
def update_result(result_id: int, payload: dict, institute=Depends(main.get_current_institute)):
    ...
    write_access(institute, row["branch_id"])
    value = payload.get("marks")
    marks = None if value in (None, "") else float(value)
    cur.execute("SELECT overall_marks FROM exam_history WHERE id=%s", (row["history_id"],)); maximum = float(cur.fetchone()[0])
    ...
    cur.execute("UPDATE exam_results SET marks=%s, updated_at=NOW() WHERE id=%s", (marks,result_id)); conn.commit(); conn.close()
    return {"ok": True}
...
def update_history(history_id:int,payload:dict,institute=Depends(main.get_current_institute)):
    ...
    write_access(institute,row["branch_id"])
    fields={k:str(payload[k]).strip() for k in ("subject","topic","batch_name","exam_date") if k in payload}
    if "overall_marks" in payload: fields["overall_marks"]=float(payload["overall_marks"])
    allowed={"subject","topic","batch_name","exam_date","overall_marks"}; fields={k:v for k,v in fields.items() if k in allowed}
    if fields:
        set_sql=", ".join(f"{k}=%s" for k in fields); cur.execute(f"UPDATE exam_history SET {set_sql},updated_at=NOW() WHERE id=%s",(*fields.values(),history_id))
