...
function forceMainCampus(){const sel=$('#branchSelector');if(!sel)return false;const opt=Array.from(sel.options).find(o=>String(o.textContent||'').trim().toLowerCase()==='main campus');if(!opt)return false;if(!initialBranchDone){initialBranchDone=true;if(window.currentBranchId!==undefined)window.currentBranchId=opt.value;try{localStorage.setItem('currentBranchId',opt.value)}catch(e){}sel.value=opt.value;sel.dispatchEvent(new Event('change',{bubbles:true}));}return true}
const branchTimer=setInterval(()=>{if(forceMainCampus())clearInterval(branchTimer)},250);setTimeout(()=>{if(!initialBranchDone)forceMainCampus()},10000);
function installClock(){const candidates=$$('h1,h2,h3,div,p,span').filter(e=>/^welcome\s+/i.test(text(e))&&e.children.length<6);const welcome=candidates.sort((a,b)=>a.textContent.length-b.textContent.length)[0];if(!welcome||welcome.querySelector('.ibex-clock')||$('#ibexClock'))return;const clock=document.createElement('span');clock.id='ibexClock';clock.className='ibex-clock';const time=document.createElement('span');time.className='ibex-clock-time';const date=document.createElement('span');date.className='ibex-clock-date';clock.append(time,date);welcome.appendChild(clock);function tick(){const d=new Date();time.textContent=new Intl.DateTimeFormat('en-IN',{timeZone:'Asia/Kolkata',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:true}).format(d)+' IST';date.textContent=new Intl.DateTimeFormat('en-IN',{timeZone:'Asia/Kolkata',weekday:'long',day:'2-digit',month:'short',year:'numeric'}).format(d)}tick();setInterval(tick,1000)}
function trimHomeStats(){const main=$('#mainContent');if(!main)return;$$('.glass-panel').forEach(card=>{if(!main.contains(card))return;const t=text(card).toLowerCase();if(!t||/parallax|institute search|welcome/.test(t))return;if(/ongoing\s+lectures?/.test(t)){card.classList.add('ibex-home-ongoing');return}if(/total\s+(students?|teachers?|classrooms?)|attendance\s*(rate|today|overview)?|pending\s*(fees|payments?)|fees?\s*(pending|collected|due)|students?\s*(present|absent|enrolled)|analytics|overview|summary/.test(t)&&t.length<900)card.classList.add('ibex-hidden-home-stat')})}
function installSearch(){const main=$('#mainContent');if(!main||$('#ibexParallaxSearch'))return;const welcome=$$('#mainContent h1,#mainContent h2,#mainContent h3').find(e=>/^welcome\s+/i.test(text(e)));const host=welcome?.closest('.glass-panel')||main.querySelector('.glass-panel');if(!host)return;const box=document.createElement('section');box.id='ibexParallaxSearch';box.className='ibex-parallax-search';box.innerHTML='<label>Parallax · Institute Search</label><div class="ibex-parallax-row"><input id="ibexSearchInput" placeholder="Search students, teachers, attendance, fees, classrooms…"><button type="button">Search</button></div><div id="ibexSearchResults"></div>';host.after(box);box.querySelector('button').onclick=runSearch;box.querySelector('input').onkeydown=e=>{if(e.key==='Enter')runSearch()}}
