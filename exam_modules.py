...
def _check(institute,module='results'): main.check_module_access(institute,module)
class ResultGenerateRequest(BaseModel):
    branch_id:int; batch_name:str; subjects:str; topics:str; exam_date:str; overall_marks:float
class ResultRecordUpdate(BaseModel):
    subjects:str; topics:str; exam_date:str; overall_marks:float; marks:float|None=None
class HistoryCreateRequest(BaseModel):
    branch_id:int; subject:str; topic:str; batch_name:str; exam_date:str
class HistoryUpdateRequest(BaseModel):
    subject:str; topic:str; batch_name:str; exam_date:str
...
def get_results(branch_id:int,batch_name:str='',institute=Depends(main.get_current_institute)):
    _check(institute,'results'); main.verify_branch_read_access(branch_id,institute.id); conn=main.get_conn()
    try:
        cur=conn.cursor(); scope='branch_id IN (SELECT id FROM branches WHERE tenant_id=%s)' if branch_id==0 else 'branch_id=%s'; params=[institute.id if branch_id==0 else branch_id]
        if batch_name.strip(): scope+=' AND batch_name=%s'; params.append(batch_name.strip())
        cur.execute(f'SELECT * FROM exam_results WHERE {scope} ORDER BY exam_date DESC,batch_name,student_name,id',params); return [dict(r) for r in cur.fetchall()]
    finally: conn.close()
...
def get_result_batches(branch_id:int,institute=Depends(main.get_current_institute)):
    _check(institute,'results'); main.verify_branch_read_access(branch_id,institute.id); conn=main.get_conn()
    try:
        cur=conn.cursor(); q='SELECT DISTINCT batch FROM students WHERE branch_id IN (SELECT id FROM branches WHERE tenant_id=%s) AND COALESCE(batch,\'\')<>\'\' ORDER BY batch' if branch_id==0 else 'SELECT DISTINCT batch FROM students WHERE branch_id=%s AND COALESCE(batch,\'\')<>\'\' ORDER BY batch'; cur.execute(q,(institute.id if branch_id==0 else branch_id,)); return [r[0] for r in cur.fetchall()]
    finally: conn.close()
...
def generate_results(req:ResultGenerateRequest,institute=Depends(main.require_write_access)):
    _check(institute,'results'); main.verify_branch_ownership(req.branch_id,institute.id); batch=req.batch_name.strip(); subjects=req.subjects.strip(); topics=req.topics.strip(); date=req.exam_date.strip()
    if not batch or not subjects or not topics or not date or req.overall_marks<=0: raise HTTPException(status_code=400,detail='Batch, subject(s), topic(s), exam date and a positive overall marks value are required.')
    conn=main.get_conn()
    try:
        cur=conn.cursor(); cur.execute('SELECT id,name,roll_number FROM students WHERE branch_id=%s AND batch=%s ORDER BY LOWER(name)',(req.branch_id,batch)); students=cur.fetchall()
        if not students: raise HTTPException(status_code=400,detail=f'No students found in batch "{batch}".')
        ...
        for s in students: cur.execute('INSERT INTO exam_results(branch_id,batch_name,subjects,topics,exam_date,overall_marks,student_id,student_name,marks) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,NULL)',(req.branch_id,batch,subjects,topics,date,req.overall_marks,s['id'],s['name']))
        conn.commit(); cur.execute('SELECT * FROM exam_results WHERE branch_id=%s AND batch_name=%s AND subjects=%s AND topics=%s AND exam_date=%s ORDER BY LOWER(student_name)',(req.branch_id,batch,subjects,topics,date)); return [dict(r) for r in cur.fetchall()]
    finally: conn.close()
...
def update_result(record_id:int,req:ResultRecordUpdate,institute=Depends(main.require_write_access)):
    _check(institute,'results'); conn=main.get_conn()
    try:
        ...
        if req.overall_marks<=0 or (req.marks is not None and (req.marks<0 or req.marks>req.overall_marks)): raise HTTPException(status_code=400,detail='Marks must be between 0 and the overall marks.')
        cur.execute('UPDATE exam_results SET subjects=%s,topics=%s,exam_date=%s,overall_marks=%s,marks=%s WHERE id=%s',(req.subjects.strip(),req.topics.strip(),req.exam_date.strip(),req.overall_marks,req.marks,record_id)); conn.commit(); return {'status':'updated'}
    finally: conn.close()
...
def get_exam_history(branch_id:int,institute=Depends(main.get_current_institute)):
    _check(institute,'history'); main.verify_branch_read_access(branch_id,institute.id); conn=main.get_conn()
    try:
        cur=conn.cursor(); scope='branch_id IN (SELECT id FROM branches WHERE tenant_id=%s)' if branch_id==0 else 'branch_id=%s'; cur.execute(f'SELECT * FROM exam_history WHERE {scope} ORDER BY exam_date DESC,id DESC',(institute.id if branch_id==0 else branch_id,)); return [dict(r) for r in cur.fetchall()]
    finally: conn.close()

@app.post('/api/exams/history')
def create_exam_history(req:HistoryCreateRequest,institute=Depends(main.require_write_access)):
    _check(institute,'history'); main.verify_branch_ownership(req.branch_id,institute.id); vals=[x.strip() for x in (req.subject,req.topic,req.batch_name,req.exam_date)]
